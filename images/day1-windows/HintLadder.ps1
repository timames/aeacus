...
function Get-Token($category, $tier) {
    ...
    $bytes = [System.Text.Encoding]::UTF8.GetBytes("${Salt}:${category}:${tier}")
...
$Hints = @{
    "secpol" = @{
        ...
        1 = "Where would a company set rules about how long passwords must be?"
        2 = "Try running secpol.msc. Look under Account Policies and Local Policies."
        3 = "Account Policies covers passwords and lockout. Local Policies > Audit Policy for auditing. Security Options: look for anything with 'logon' in the name."
    }
    "smbv1" = @{
        Label = "SMBv1"
        1 = "Old protocols are vulnerabilities too. Google 'WannaCry 2017' and ask what it exploited."
        2 = "Where do you turn Windows components on and off? Try optionalfeatures.exe or search 'Turn Windows features on or off'."
        3 = "Look for 'SMB 1.0/CIFS File Sharing Support' in Windows Features and uncheck it."
    }
    "remotereg" = @{
        Label = "Remote Registry"
        1 = "Which services let someone mess with this machine remotely?"
        2 = "Open services.msc, sort by name, look for anything with 'Remote' in the name."
        3 = "Remote Registry service: stop it and set Startup Type to Disabled."
    }
    "uac" = @{
        Label = "UAC"
        1 = "Run something as admin. Did Windows ask you first? Should it have?"
        2 = "Search for 'UAC' in the Start menu, or check Control Panel > User Accounts."
    ...
    "forensics1" = @{
        Label = "Forensics Q1 / Scheduled Task"
        1 = "Backdoors need to survive a reboot. How does Windows run things automatically?"
        2 = "Open Task Scheduler. Read the Actions tab of each task, not just the name."
        3 = "Task Scheduler > look for a task called 'Updater'. Its action runs ncat.exe on port 4444. Bonus: netstat -ano | findstr LISTENING to confirm."
    }
    "bkahale" = @{
        Label = "bkahale password"
        1 = "Read the README carefully. Does anything in it make you cringe?"
        2 = "Look at the passwords listed for authorized users. Would you use any of those passwords for your bank?"
        3 = "bkahale's password is 'password'. Change it with: net user bkahale <NewStrongPassword>"
    }
}

$CategoryOrder = @("secpol","smbv1","remotereg","uac","forensics1","bkahale")
